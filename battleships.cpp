// Final implementaion of the Battleships Project
...
private:
	// Declare game boards
	BattleshipSquare squares[boardSize][boardSize];
...
public:
	...
	void ResetBoard()
	{
		for (int i = 0; i < 8; ++i)
		{
			for (int j = 0; j < 8; ++j)
			{
				squares[i][j].ResetSquare();
			}
		}
	}

	void DisplayBoard(bool firingBoard)
	{
		char currentSquare;
		if (firingBoard == false) // Placing board
		{
			...
			for (int i = 0; i < boardSize; i++)
			{
				...
				{
					currentSquare = GetSquareState(i, j);
					cout << " " << currentSquare << " ";
				}
				cout << endl;
			}
	...
	bool PlaceShip(int column, int row, int direction, Ship* ship)
	{

		for (int i = 0; i < ship->GetSize(); i++)
		{
			this->squares[row][column].AddShip(ship);
	...
	bool FireAtSquare(int column, int row)
	{
		...
		if (squares[row][column].GetStatus() == ' ' || squares[row][column].GetStatus() == 'S')
		{
			squares[row][column].Fire();
			validShot = true;
		}
		else
		{
	...
	bool PlacementCheck(int column, int row, int direction, int size)
	{
		...
		{
			if (GetSquareState(row, column) != ' ' || column < 0 || column > 8 || row < 0 || row > 8)
			{
				return false;
			}
...
public:
	...
	{
		this->board = Board();
		this->ships[0] = new Ship(2, "Destroyer");
		this->ships[1] = new Ship(3, "Submarine");
		this->ships[2] = new Ship(3, "Cruiser");
		this->ships[3] = new Ship(4, "Battleship");
		this->ships[4] = new Ship(5, "Carrier");
	}
	...
	void PlaceShip(int column, int row, int direction, int shipIndex)
	{
		board.PlaceShip(column, row, direction, ships[shipIndex]);
	}
	...
	void DisplayBoard(int& turnNumber, bool firingBoard)
	{
		...
		board.DisplayBoard(firingBoard);
...
int main() {
	...
	else
	{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				while (true)
				{
					...
					if (players[p].PlacementCheck(column, row, direction, size))
					{
						...
						players[p].PlaceShip(column, row, direction, i);
		...
		while (true)
		{
			...
			// check if target player lost (shooting player won)
